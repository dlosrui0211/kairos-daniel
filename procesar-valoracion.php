...
try {
    ...
    if (strpos($e->getMessage(), 'foreign key') !== false || strpos($e->getMessage(), 'FOREIGN KEY') !== false || strpos($e->getMessage(), 'a]foreign key') !== false) {
        $mensaje = 'El producto no existe en la base de datos. No se puede valorar.';
    }
    echo json_encode([
        'success' => false,
        'message' => $mensaje
